...
function PropertyForm({ form, setForm, mainImage, setMainImage, extraImages, setExtraImages, setResult, onCreated }) {
  ...
  return (
    <section className="surface-card property-form-card compact-form-card">
      ...
      <form className="profile-form property-form-density briefing-accordion-form" onSubmit={handleSubmit}>
        <details className="briefing-form-section" name="briefing-form-section" open>
          <summary><strong>기본 설정</strong><span>{form.template_type === "detail" ? "상세형" : "1페이지형"} · {form.deal_type || "거래유형 선택"}</span></summary>
        ...
        <details className="briefing-form-section" name="briefing-form-section">
          <summary><strong>사진</strong><span>대표 {mainImage ? "1장" : "없음"} / 추가 {extraImages.length}장</span></summary>
          <div className="briefing-section-body">
        ...
        <details className="briefing-form-section" name="briefing-form-section">
          <summary><strong>가격 / 주소</strong><span>{pricePreview || "가격 미입력"} · {form.address || "주소 미입력"}</span></summary>
          <div className="briefing-section-body">
        ...
        <details className="briefing-form-section" name="briefing-form-section">
          <summary><strong>상세 정보</strong><span>{[form.exclusive_area && `전용 ${form.exclusive_area}${form.exclusive_area_unit}`, form.elevator && `엘리베이터 ${form.elevator}`, form.parking_count && `주차 ${form.parking_count}`].filter(Boolean).join(" · ") || "면적 / 층수 / 주차 입력"}</span></summary>
          <div className="briefing-section-body">
        ...
        <details className="briefing-form-section" name="briefing-form-section">
          <summary><strong>소개 문구 / 태그</strong><span>{form.recommended_use || form.description || "추천 업종과 설명 입력"}</span></summary>
          <div className="briefing-section-body">
            <div className="field-grid">
        ...
        <details className="briefing-form-section" name="briefing-form-section">
          <summary><strong>담당자 정보</strong><span>{[form.contact_name, form.contact_phone].filter(Boolean).join(" · ") || "담당자명 / 연락처"}</span></summary>
          <div className="briefing-section-body">
            <div className="field-grid">
        ...
        <div className="briefing-sticky-actions">
          <button type="button" className="secondary-btn" onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}>미리보기 확인</button>
